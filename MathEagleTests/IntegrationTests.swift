...
class IntegrationTests: XCTestCase {
    ...
    func testMidpoint() {
        ...
        XCTAssertEqualWithAccuracy(I, 2.0, ACCURACY)
        
        I = Integration.midpoint(0.0, M_PI){ sin($0) }
        XCTAssertEqualWithAccuracy(I, 2.0, ACCURACY)
        
        I = Integration.midpoint(1.0, 3.0){ log($0) }
        XCTAssertEqualWithAccuracy(I, 1.295836867, ACCURACY)
    }
    ...
    func testAdaptiveSimpsonPerformance() {
        ...
        println("Adaptive simpson time = \(time)\nBaseline = \(baseline)\n\(baseline/time) times faster than baseline")
    }
